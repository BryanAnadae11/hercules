...
def contact(request):
	...
	context={'form':form}
	return render(request, 'herculesbankapp/contact.html', context)


def about(request):
	return render(request, 'herculesbankapp/about.html')
...
def foreign_transaction(request):
	...
	if request.method == 'POST' and canClientTransfer:
		...
		if foreign_transaction and float(foreign_transaction_number):
			...
			email_message.send()
			return render(request, 'herculesbankapp/transaction_proof.html', {'amount_sent':amount_sent, 'bank_name':bank_name, 'account_number':account_number})
		else:
...
def admincreditaccount(request, pk):
	...
	if request.method == 'POST':
		...
		if amount:
			...
			template= render_to_string('herculesbankapp/creditalert.html', {'name':firstName, 'newacc_bal':newacc_bal, 'acc_currency':acc_currency})
			plain_message= strip_tags(template)
			email_message= EmailMultiAlternatives(
...
def admindebitaccount(request, pk):
	...
	if request.method == 'POST':
		...
		if float(client_deposit) > float(amount):
			...
			client_info= Client.objects.filter(id=client_id)
			client_info.update(deposit=newacc_bal)
			template= render_to_string('herculesbankapp/debitalert.html', {'name':firstName, 'newacc_bal':newacc_bal, 'acc_currency':acc_currency})
			plain_message= strip_tags(template)
			email_message= EmailMultiAlternatives(
...
def verify_otp(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if otp_obj.otp_code == entered_otp and not otp_obj.is_expired():
                login(request, user)
